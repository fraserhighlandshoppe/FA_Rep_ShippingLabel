...
- **No Quick Fix Workarounds**: Don't do a quick fix insert of a line or two when the full to standards listed here fix is only a small number of lines.
- **No major code removal round 1**: When removing code, don't remove in 1st instance.  Comment out the code.  Code being replaced should probably be in an SRP class, so: 
write the new code; comment the old code and add a line saying where the new code can be found; and insert the call to the new class.
- **Use Best Practices**: Always use best practices.  Don't do quick fixes "to get it to work".
- **Avoid empty scaffolding**:  Always write as much code as you can for the class or function.  Don't just put
a class with an empty function and a comment to write the code later.  If you knew you'd need to function you
must have an idea what for.

### Libraries
- **Use Battle Tested Libraries**: Use Composer/Packagist libraries where possible
- **Use Libraries rather than re-invent the wheel**:Use well known libraries like Pear, Symfony, Monolog rather than re-creating.
- **Use OUR Libraries**: Use ksfraser libraries where available.  They are hosted on github but made into Packagist packages.
...
- **Write BABOK documents**: We want the full suite: Business requirements, Use cases, 
Functional Requirements, Non Functional Requirements, System Architecture, Security,
Test Plans, Requirements Traceability Matrix.
- **Cross Reference Code to Requirements and Tests**: Each function and each class should
have a requirement somewhere that drives its existance.  RTM traces requirement to test
to code (file/class/function).  UML sequence diagrams, message flows, etc where applicable.